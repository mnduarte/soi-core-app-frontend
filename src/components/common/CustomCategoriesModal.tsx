...
// Personaliza las categorías de fotos del consultorio (settings.photoCategories).
// Guarda solo (al agregar/quitar), sin botón Guardar. Vacío → el front usa los
// defaults (Intraoral / Extraoral / Radiografía); por eso al abrir seedeamos la
// lista efectiva para poder editarla.
export function CustomCategoriesModal({
  ...
  const saveMut = useMutation({
    mutationFn: (items: string[]) => clinicsApi.updateSettings({ photoCategories: items }),
  ...
  const add = () => {
    ...
    commit([...list, v], `"${v}" agregada ✓`);
  };
  const remove = (c: string) => commit(list.filter(x => x !== c), `"${c}" quitada`);
    ...
    >
      ...
      >
        <div style={{ padding: '18px 20px 6px', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 16, fontWeight: 600 }}>Personalizar categorías</div>
            <div style={{ fontSize: 12.5, color: 'var(--text-tertiary)', marginTop: 3 }}>
              Se guardan solas. Se usan al subir y filtrar fotos.
            </div>
          </div>
        ...
        <div style={{ padding: '10px 20px', display: 'flex', flexWrap: 'wrap', gap: 6, maxHeight: 220, overflowY: 'auto' }}>
          {list.length === 0 && (
            <div style={{ fontSize: 12.5, color: 'var(--text-tertiary)', padding: '4px 0' }}>Sin categorías. Agregá abajo.</div>
          )}
          {list.map(c => (
            <span
              key={c}
        ...
        <div style={{ padding: '8px 20px 20px', display: 'flex', gap: 8 }}>
          <input
            className="input"
            placeholder="Nueva categoría (ej: Panorámica)"
            value={nuevo}
